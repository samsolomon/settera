# Settara — Settings Framework Specification v0.1

## 1. Overview

Settara is an open-source, schema-driven settings framework for React applications. It provides a complete system for building settings UIs — from small startups to enterprise applications — with keyboard navigation, search, nested layouts, and responsive design built in.
...
The headless layer also exposes composable primitives (`<SettaraSidebar>`, `<SettaraContent>`, `<SettaraSearch>`, `<SettaraSettingRow>`). `<SettaraRenderer>` composes them into a default layout. Developers who need control over layout use the parts directly.
...
const { schema, values, setValue, validate } = useSettara();
...
```tsx
import { SettaraProvider } from "@settara/react";
...
<SettaraProvider schema={schema} renderers={customRenderers}>
  <SettaraRenderer ... />
</SettaraProvider>
```
...
The 768px breakpoint is the default. The styled layer allows overriding this. The headless layer exposes a `useSettaraLayout()` hook that returns `"desktop" | "mobile"` so custom renderers can adapt.
...
│   │   │   └── index.ts
│   │   └── package.json     # @settara/schema
│   │
│   ├── react/               # Headless React layer
│   │   ├── src/
│   │   │   ├── provider.tsx          # SettaraProvider context
│   │   │   ├── renderer.tsx          # SettaraRenderer component
│   │   │   ├── hooks/
│   │   │   │   ├── useSettara.ts
│   │   │   │   ├── useSettaraSearch.ts
│   │   │   │   ├── useSettaraNavigation.ts
│   │   │   │   ├── useSettaraSetting.ts
│   │   │   │   └── useSettaraLayout.ts
│   │   │   ├── components/
│   │   │   │   ├── SettaraSidebar.tsx
│   │   │   │   ├── SettaraContent.tsx
│   │   │   │   ├── SettaraSearch.tsx
│   │   │   │   └── SettaraSettingRow.tsx
│   │   │   ├── keyboard/            # Focus management, keyboard handlers
│   │   │   └── index.ts
│   │   └── package.json     # @settara/react
│   │
...
2. **Package naming** → `@settara/schema`, `@settara/react`, `@settara/ui` (scoped npm). Consistent with Radix (`@radix-ui/*`), prevents name squatting.
